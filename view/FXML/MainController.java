...
public class MainController implements Initializable {
	...
	public void applyRule(ActionEvent event){
		...
		if(!tempFeedbackCategory.getRules().isEmpty()){
			controller.addRuleToCategory(debetOrCredit,getSelectedCategory().getName(), tempFeedbackCategory.getRules().get(0));
			controller.checkTransactionsAgianstRules("not yet assigned");
		}
	...
	public void applyRuleOverwrite(ActionEvent event){
		if(chcKeyWord.isSelected()){
			controller.addKeyWordToCategory(debetOrCredit,getSelectedCategory().getName(), txtKeyword.getText());
			controller.checkTransactionsAgainstKeyWords(getSelectedTransaction().getCategory().getName());
		}
		if(chcAccount.isSelected()){
			controller.addAccountToCategory(debetOrCredit, getSelectedCategory().getName(), txtCounterAccount.getText());
	...
	public void showRule(){
		...
		if(findAppliccableKeyword()){
			for(String s : getSelectedCategory().getKeyWords()){
				backSearchController.olKeyWords.add(s);
			}
			backSearchController.initKeywords();	
	...
	public void deleteRule(){
		...
		if(findAppliccableKeyword()){
			System.out.println("ind delte rule sel cat "+getSelectedCategory());
